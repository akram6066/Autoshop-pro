  ...
  return (
    <div className="w-full h-full flex bg-[#050505] text-zinc-300 font-sans overflow-hidden">
      ...
      <div className="w-16 border-r border-zinc-800/80 bg-[#020202] flex flex-col items-center py-6 gap-8 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
        <div className="w-8 h-8 rounded bg-gradient-to-tr from-brand-600 to-brand-400 flex items-center justify-center text-white font-bold shadow-[0_0_15px_rgba(99,102,241,0.4)]">
          A
        </div>
        <div className="w-10 h-10 rounded-xl hover:bg-zinc-800/30 flex items-center justify-center text-zinc-500 transition-colors">
          <ShoppingCart size={20} />
        </div>
        <div className="w-10 h-10 rounded-xl bg-zinc-800/50 flex items-center justify-center text-brand-400 border border-brand-500/20 shadow-[0_0_15px_rgba(99,102,241,0.15)]">
          <BarChart2 size={20} />
        </div>
        <div className="w-10 h-10 rounded-xl hover:bg-zinc-800/30 flex items-center justify-center text-zinc-500 transition-colors">
          <Search size={20} />
...
function ProductCard({
  ...
  return (
    <div className="bg-[#0a0a0a] border border-zinc-800/80 rounded-xl p-4 hover:border-brand-500/50 cursor-pointer transition-all duration-300 group relative overflow-hidden shadow-lg hover:shadow-[0_0_20px_rgba(99,102,241,0.15)]">
      ...
      <div className="text-[10px] uppercase tracking-wider text-brand-400 font-bold mb-1">
        {category}
      </div>
      <div className="text-sm text-zinc-200 font-semibold leading-tight mb-4 h-10 group-hover:text-white transition-colors">
        {name}
      </div>
  ...
  return (
    <div className="flex justify-between items-start group">
      <div className="flex-1">
        <div className="text-sm font-medium text-zinc-200 line-clamp-1 group-hover:text-white transition-colors">
          {name}
        </div>
        <div className="text-xs text-zinc-500 mt-2 flex items-center gap-2">
          <button className="w-6 h-6 bg-zinc-800 rounded flex items-center justify-center hover:bg-zinc-700 transition-colors">
            <Minus size={12} />
          </button>
          <span className="text-zinc-300 font-medium w-4 text-center">
            {qty}
          </span>
          <button className="w-6 h-6 bg-zinc-800 rounded flex items-center justify-center hover:bg-zinc-700 transition-colors">
            <Plus size={12} />
