...
export default function SetupPage() {
  ...
  async function handleCreateShop(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (userError || !user) { setShopError("Not logged in"); return; }
  ...
  async function handleCreateRooms(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      await Promise.all([
        ...
        supabase.rpc("seed_default_categories", { p_shop_id: createdShop!.id })
          .then(({ error: e }) => { if (e) console.error("[setup] seed categories:", e); }),
      ]);
      ...
      const updatedShops: ShopWithRole[] = [...existingShops.filter((s) => s.id !== newShop.id), newShop];
          ...
          <div key={s.key} className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              ...
              <span
                ...
                style={{
                  color: i <= currentStepIdx ? "var(--color-ink-primary)" : "var(--color-ink-ghost)",
            ...
            {i < steps.length - 1 && (
              <div
                className="w-8 h-px"
                style={{ background: i < currentStepIdx ? "var(--color-brand-400)" : "oklch(88% 0 0)" }}
              />
            )}
          </div>
        ))}
        ...
        {step === "rooms" && (
          <form onSubmit={handleCreateRooms} className="p-6">
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={addRoom} className="btn btn-secondary">
                Add
              </button>
            </div>
            {roomsError && (
        ...
        {step === "done" && (
          <div className="p-8 text-center animate-scale-in">
            ...
            <p className="text-sm" style={{ color: "var(--color-ink-secondary)" }}>
              {isAddingNew ? "Taking you to your overview…" : "Taking you to your dashboard…"}
            </p>
          </div>
        )}
      </div>
