    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--color-ink-tertiary)" }}>
            ...
            {lowStockCount > 0 && (
              <span className="badge badge-warning ml-2">{lowStockCount} low stock</span>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <select
          ...
          value={sortBy}
          onChange={(e) => setSortBy(e.target.value as "name" | "qty" | "price")}>
      ...
      {isLoading ? (
        <div className="card">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-14 mx-4 my-2 rounded-lg animate-pulse-soft"
              style={{ background: "var(--color-surface-2)" }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="card p-12 text-center">
          <p className="text-lg mb-1" style={{ color: "var(--color-ink-secondary)" }}>No products found</p>
          ...
          {!search && roomFilter === "all" && categoryFilter === "all" && (
            <Link href="/inventory/new" className="btn btn-primary btn-sm">Add product</Link>
          )}
        </div>
      ) : (
              ...
              {filtered.map((product) => (
                <tr key={product.id}>
                  ...
                  <td style={{ color: "var(--color-ink-secondary)", fontSize: 13 }}>
                    {product.size || <span style={{ color: "var(--color-ink-ghost)" }}>—</span>}
                  </td>
                  <td>
                    <span className="badge badge-neutral">{CATEGORY_LABELS[product.category]}</span>
                  </td>
                  ...
                  <td>
                    <StockBadge qty={product.quantity} minStock={product.min_stock} />
                  </td>
                  <td>
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => {
                          if (shopId && confirm(`Delete "${product.name}"?`)) {
                            deleteProduct({ shopId, productId: product.id });
                          }
                        }}
                        className="btn btn-ghost btn-sm btn-icon"
