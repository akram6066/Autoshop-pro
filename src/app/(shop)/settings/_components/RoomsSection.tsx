...
export function RoomsSection() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setMsg("");
    const { data, error } = await supabase.from("rooms").insert({ shop_id: shopId, name }).select().single();
  ...
  async function handleDelete(roomId: string) {
    ...
    if ((count ?? 0) > 0) { setMsg(`Cannot delete — ${count} product(s) in this room. Move them first.`); return; }
  ...
  return (
    <>
        ...
        <div className="mb-4">
          {rooms.map((room) => (
            <div key={room.id} className="flex items-center justify-between py-2.5"
              ...
              <button onClick={() => handleDelete(room.id)} className="btn btn-ghost btn-sm btn-icon">
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        ...
        <button type="submit" className="btn btn-secondary" disabled={!newRoomName.trim()}>Add room</button>
      </form>
