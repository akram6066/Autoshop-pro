...
function startOf(d: Date) { const c = new Date(d); c.setHours(0,0,0,0); return c.toISOString(); }
function endOf(d: Date) { const c = new Date(d); c.setHours(23,59,59,999); return c.toISOString(); }
  ...
  return (
    <div className="flex items-end gap-1 h-40 overflow-x-auto pb-2">
      {data.map((row) => {
        ...
        return (
          <div key={row.date} className="flex flex-col items-center gap-1 flex-1 min-w-[28px] group">
            <div className="relative w-full">
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10"
                style={{ background: "var(--color-ink-primary)", color: "white" }}>
              ...
              <div
                ...
                style={{
                  height: `${Math.max(pct * 1.3, 4)}px`,
                  background: row.total_revenue > 0 ? "var(--color-brand-400)" : "var(--color-surface-3)",
                }}
              />
            </div>
...
export default function ReportsPage() {
  ...
  function exportCSV() {
    const rows = [
      ...
      ...summary.map((r) => [r.date, r.total_revenue.toFixed(2), r.order_count]),
    ];
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <button onClick={exportCSV} className="btn btn-secondary btn-sm" disabled={summary.length === 0}>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        {(["7d", "30d", "90d", "custom"] as Range[]).map((r) => (
          <button
            ...
            style={{
              background: range === r ? "var(--color-brand-500)" : "var(--color-surface-0)",
              color: range === r ? "white" : "var(--color-ink-secondary)",
              border: `1px solid ${range === r ? "var(--color-brand-600)" : "oklch(88% 0.006 250)"}`,
            }}>
            {RANGE_LABELS[r]}
          </button>
        ))}
        ...
        {range === "custom" && (
          <div className="flex items-center gap-2 ml-2">
            ...
            />
            <span style={{ color: "var(--color-ink-tertiary)", fontSize: 13 }}>to</span>
            <input
              type="date"
              className="input"
              style={{ width: 150, height: 32, fontSize: 13 }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 stagger">
        {[
          ...
          { label: "Active days", value: `${totals.activeDays} / ${summary.length}` },
        ].map((kpi) => (
          <div key={kpi.label} className="card p-4 animate-fade-in-up">
            <p className="text-xs uppercase tracking-widest mb-2" style={{ color: "var(--color-ink-tertiary)" }}>
              {kpi.label}
            </p>
            <p className="text-2xl font-semibold" style={{ color: "var(--color-ink-primary)" }}>
              {isLoading ? "—" : kpi.value}
            </p>
          </div>
        ))}
      ...
      {/* Chart */}
      <div className="card p-5 mb-8 animate-fade-in-up" style={{ animationDelay: "200ms" }}>
        <h2 className="font-medium text-sm mb-4" style={{ color: "var(--color-ink-secondary)" }}>
          Daily revenue
        </h2>
        {isLoading ? (
          <div className="h-40 rounded-lg animate-pulse-soft" style={{ background: "var(--color-surface-2)" }} />
        ) : summary.length === 0 ? (
          <div className="h-40 flex items-center justify-center">
            <p className="text-sm" style={{ color: "var(--color-ink-tertiary)" }}>No sales data for this period</p>
          </div>
        ) : (
