...
BEGIN
  ...
  IF p_variant_id IS NOT NULL THEN
    ...
    VALUES (
      p_dest_shop_id, 
      'Incoming Transfer', 
      'Received ' || p_quantity || ' units of ' || v_name || ' (Size: ' || v_var_size || ', SKU: ' || COALESCE(v_var_sku, v_sku, 'N/A') || ') from ' || v_source_shop_name || ' (' || v_source_room_name || ')'
    );
  ...
  ELSE
    ...
    VALUES (
      p_dest_shop_id, 
      'Incoming Transfer', 
      'Received ' || p_quantity || ' units of ' || v_name || ' (Size: ' || COALESCE(v_size, 'N/A') || ', SKU: ' || COALESCE(v_sku, 'N/A') || ') from ' || v_source_shop_name || ' (' || v_source_room_name || ')'
    );
