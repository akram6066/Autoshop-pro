...
-- RPC to persist it atomically.
-- ============================================================

ALTER TABLE public.sales
ADD COLUMN IF NOT EXISTS payment_method text NOT NULL DEFAULT 'cash'
  CHECK (payment_method IN ('cash', 'mpesa', 'credit'));

CREATE INDEX IF NOT EXISTS idx_sales_payment_method
  ON public.sales(shop_id, payment_method);

-- Replace record_sale to include payment_method in the INSERT.
-- Preserves idempotency from 009: if the sale id already exists, return early.
...
BEGIN
  ...
  INSERT INTO public.sales (
    id, shop_id, user_id, total_amount, payment_method, synced, created_at
  )
  VALUES (
    v_sale_id,
    (p_sale->>'shop_id')::uuid,
