...
BEGIN
  ...
  IF v_source_shop_id = p_dest_shop_id THEN
    RAISE EXCEPTION 'Cannot transfer inventory within the same shop. Please edit the product to change its assigned room.';
  END IF;
  ...
  IF p_variant_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      CASE WHEN v_var_size IS NOT NULL AND v_var_size <> '' THEN ' (Size: ' || v_var_size || ')' ELSE '' END || 
      CASE WHEN COALESCE(v_var_sku, v_sku) IS NOT NULL AND COALESCE(v_var_sku, v_sku) <> '' THEN ' [SKU: ' || COALESCE(v_var_sku, v_sku) || ']' ELSE '' END || 
      ' from ' || v_source_shop_name || ' (' || v_source_room_name || '). New total stock: ' || (v_dest_qty + p_quantity)
    );
  ...
  ELSE
    ...
    VALUES (
      ...
      'Incoming Transfer', 
      'Received ' || p_quantity || ' units of ' || v_name || 
      CASE WHEN v_size IS NOT NULL AND v_size <> '' THEN ' (Size: ' || v_size || ')' ELSE '' END || 
      CASE WHEN v_sku IS NOT NULL AND v_sku <> '' THEN ' [SKU: ' || v_sku || ']' ELSE '' END || 
      ' from ' || v_source_shop_name || ' (' || v_source_room_name || '). New total stock: ' || (v_dest_qty + p_quantity)
    );
