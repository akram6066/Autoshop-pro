...
BEGIN
  ...
  IF p_variant_id IS NOT NULL THEN
    ...
    INSERT INTO public.stock_movements (
      ...
      v_source_shop_id, p_source_product_id, p_variant_id, 'OUT', p_quantity, v_source_qty - p_quantity, 'server', 'transfer', auth.uid(), true, COALESCE(p_transfer_date, now())
    );
    ...
    INSERT INTO public.stock_movements (
      ...
      p_dest_shop_id, v_dest_product_id, v_dest_variant_id, 'IN', p_quantity, v_dest_qty + p_quantity, 'server', 'transfer', auth.uid(), true, COALESCE(p_transfer_date, now())
    );
  ...
  ELSE
    ...
    INSERT INTO public.stock_movements (
      ...
      v_source_shop_id, p_source_product_id, 'OUT', p_quantity, v_source_qty - p_quantity, 'server', 'transfer', auth.uid(), true, COALESCE(p_transfer_date, now())
    );
    ...
    INSERT INTO public.stock_movements (
      ...
      p_dest_shop_id, v_dest_product_id, 'IN', p_quantity, v_dest_qty + p_quantity, 'server', 'transfer', auth.uid(), true, COALESCE(p_transfer_date, now())
    );
