-- End any automatic transaction block (e.g. from Supabase CLI)
COMMIT;

ALTER TYPE movement_reason ADD VALUE IF NOT EXISTS 'transfer';

-- Start a new transaction for the function
BEGIN;
...
BEGIN
  ...
  SELECT shop_id, sku, name, category, min_stock, price, size
  INTO v_source_shop_id, v_sku, v_name, v_category, v_min_stock, v_price, v_size
  ...
  IF p_variant_id IS NOT NULL THEN
    ...
    INSERT INTO public.stock_movements (
      shop_id, product_id, variant_id, type, delta, snapshot_qty, device_id, reason, user_id, synced
    ) VALUES (
      v_source_shop_id, p_source_product_id, p_variant_id, 'OUT', p_quantity, v_source_qty - p_quantity, 'server', 'transfer', auth.uid(), true
    );
    ...
    INSERT INTO public.stock_movements (
      ...
      p_dest_shop_id, v_dest_product_id, v_dest_variant_id, 'IN', p_quantity, v_dest_qty + p_quantity, 'server', 'transfer', auth.uid(), true
    );
  ...
  ELSE
    ...
    INSERT INTO public.stock_movements (
      shop_id, product_id, type, delta, snapshot_qty, device_id, reason, user_id, synced
    ) VALUES (
      v_source_shop_id, p_source_product_id, 'OUT', p_quantity, v_source_qty - p_quantity, 'server', 'transfer', auth.uid(), true
    );
    ...
    INSERT INTO public.stock_movements (
      ...
      p_dest_shop_id, v_dest_product_id, 'IN', p_quantity, v_dest_qty + p_quantity, 'server', 'transfer', auth.uid(), true
    );
